...
// GetTeamRoster returns a team's roster for the given week.
func (c *Client) GetTeamRosterByWeek(teamKey string, week int) ([]Player, error) {
	content, err := c.GetFantasyContent(
...
func (c *Client) GetAllTeams(leagueId string) ([]Team, error) {
	leagueKey := fmt.Sprintf("%s.l.%s", YearKeys["2021"], leagueId)
	content, err := c.GetFantasyContent(
